...
export class KeywordRuntimeService {
  ...
  squad(source,times=0){ const e=this.engine,n=Math.max(0,Number(times)); const made=[];for(let i=0;i<n;i++){const c=structuredClone(source);c.instanceId=`squad-${source.instanceId}-${e.state.turn}-${i}`;c.token=true;c.zone='battlefield';e.state.players[source.controller].battlefield.push(c);made.push(c);}return made; }
  ...
  amass(source,subtype='Orc',amount=1){ const p=this.engine.state.players[source.controller];let army=(p.battlefield||[]).find(x=>/Army/i.test(this.definition(x).typeLine||x.typeLine||''));if(!army){army={instanceId:`army-${source.instanceId}-${this.engine.state.turn}`,cardId:'Army Token',controller:source.controller,owner:source.controller,zone:'battlefield',token:true,power:0,toughness:0,counters:{},modifiers:{power:0,toughness:0,keywords:[]},typeLine:`Token Creature — ${subtype} Army`};p.battlefield.push(army);}army.counters['+1/+1']=Number(army.counters['+1/+1']||0)+Number(amount);return army; }
  collectEvidence(source,cardIds=[],amount=null){ const need=Number(amount??this.definition(source).collectEvidence?.amount??0),p=this.engine.state.players[source.controller];let total=0;const cards=[];for(const id of cardIds){const c=(p.graveyard||[]).find(x=>x.instanceId===id);if(!c)throw new Error('Illegal evidence card');total+=Number(this.definition(c).manaValue??c.manaValue??0);cards.push(c);}if(total<need)throw new Error('Insufficient evidence');for(const c of cards){p.graveyard=p.graveyard.filter(x=>x!==c);p.exile ||= [];c.zone='exile';p.exile.push(c);}return total; }
  ...
  soulshift(source,targetId=null,n=null){const k=Number(n??this.definition(source).soulshift??0),p=this.engine.state.players[source.controller];const legal=(p.graveyard||[]).filter(c=>/Spirit/i.test(this.definition(c).typeLine||'')&&Number(this.definition(c).manaValue??0)<=k);if(!targetId)return legal.map(c=>c.instanceId);const c=legal.find(x=>x.instanceId===targetId);if(!c)throw new Error('Illegal soulshift target');p.graveyard=p.graveyard.filter(x=>x!==c);p.hand ||= [];c.zone='hand';p.hand.push(c);return c;}
