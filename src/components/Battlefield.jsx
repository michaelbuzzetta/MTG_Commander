...
export function Battlefield({player,db,onCard,selected=()=>false,actionState=()=>null,side='player'}){
 ...
 const row=(name,items,cls)=><div key={cls} className={`permanent-row ${cls}`}>
   ...
   <div className="permanent-cards">{items.map(p=><Card key={p.instanceId} perm={p} def={db[p.cardId]} selected={selected(p.instanceId)} actionState={actionState(p)} onClick={()=>onCard?.(p)}/>)}</div>
 </div>;
